...
def test_get_proximity_entities_2d(
    ...
    api_test = api_soil
    data = api_test.get_proximity_entities_2d(api_url="test", latitude=50.1, longitude=2.22, radius=0.75)
    df = data["data"]
...
def test_get_proximity_entities_2d_wrong_data(
    ...
    with pytest.raises((APIConnectionError, DataProcessingError, InvalidParameterError)):
        api_soil.get_proximity_entities_2d(api_url="test", latitude=50, longitude=2.22, radius=0.75)
...
def test_search_any_entity_exception(api_soil: SoilAPI, mock_requests_search_any_entity: mock.Mock) -> None:
    with pytest.raises((APIConnectionError, DataProcessingError, InvalidParameterError, ValueError)):
...
def test_get_closest_entity_2d(
    ...
    dict_ = api_soil.get_closest_entity_2d(api_url="test", latitude=50.0, longitude=2.0, radius_init=10.0)
