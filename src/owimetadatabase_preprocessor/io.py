...
class API:
        ...
        if "header" in kwargs:
            ...
            if self.header and isinstance(self.header, dict) and "Authorization" in self.header:
                if not self.header["Authorization"].startswith("Token "):
                    ...
                    elif self.header["Authorization"].startswith("token") or self.header["Authorization"].startswith("Token"):
    ...
    def check_request_health(resp: requests.Response) -> None:
        ...
        if resp.status_code != 200:
            raise APIConnectionError(message=f"Error {resp.status_code}.\n{resp.reason}", response=resp)
